...
# ============ Load Data ============
df = pd.read_csv("Software_Salaries.csv")  # Adjust path if needed
...
data_drift.save_html("drift_report.html")

# 2. Model Drift
model_drift = Report(metrics=[TargetDriftPreset()])
model_drift.run(reference_data=y_train.to_frame(), current_data=y_test.to_frame())
model_drift.save_html("model_drift_report.html")

# 3. Concept Drift (RegressionPreset expects 'target' + 'prediction')
...
concept_drift.save_html("concept_drift_report.html")
